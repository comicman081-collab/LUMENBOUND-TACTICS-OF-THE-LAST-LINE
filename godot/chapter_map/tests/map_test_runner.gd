...
func _test_geometry_grounding_contract() -> void:
	...
	check(map_screen_source.contains("(52.0 if portrait else 14.0) * ui_scale") and map_screen_source.contains("detail_scroll.find_children"), "MAP_RESPONSIVE_04 portrait status rows and compact detail actions avoid overlap")
	camera_screen.free()
...
func _test_direct_move_turn_contracts() -> void:
	...
	check(not tutorial_screen.tutorial_dismiss_button.visible and tutorial_screen.tutorial_continue_button.custom_minimum_size.x <= 228.1 * portrait_scale and tutorial_screen.tutorial_body.get_theme_font_size("normal_font_size") >= roundi(23.0 * portrait_scale) and tutorial_screen.tutorial_body.get_theme_font_size("bold_font_size") == tutorial_screen.tutorial_body.get_theme_font_size("normal_font_size"), "TUTORIAL_MODAL_02 portrait keeps the 23px physical regular/bold body scale, scroll rail and centered footer inside safe modal padding")
